...
class UIO:
    ...
    def TrippleRGcoreIsGood(self, core):
        A = self.RGcoreIsGood(core[:2])
        B = not self.RGcoreIsGood(core[10:12])
        C = self.RGcoreIsGood(core[2:4])
        D = not self.RGcoreIsGood(core[6:8])
        E = self.RGcoreIsGood(core[4:6])
        F = not self.RGcoreIsGood(core[8:10])
        #print("core:", core)
        #print("A,B", A, B)
        #print("C,D", C, D)
        #print("E,F", E, F)
    ...
    def getTripplEscherCores(self, n, k, l, verbose=False, savefile=False):
        ...
        for u,v,w in pairs:
            ...
            if verbose:
                ...
                print((list(w), list(v), u), core, isgood)
                # ([0, 1], [7, 6], (5, 3, 4, 2))
            cores.append(core)
        return cores, goods
